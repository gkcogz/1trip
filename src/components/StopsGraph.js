...
export default function StopsGraph({ trip, wrapperRef, activeStopId, onActivate, onReset, printSafe = false }) {
    ...
    return (_jsxs("div", { className: `relative rounded-2xl border border-[var(--color-border)] shadow-sm p-4 overflow-visible ${printSafe ? 'bg-transparent' : 'bg-white'}`, ref: graphRef, children: [!printSafe && (_jsxs("div", { className: "flex items-center justify-between mb-3", children: [_jsx("div", { className: "text-sm font-medium text-[var(--color-accent)]", children: t('graph.header') }), onReset && (_jsx(EmojiButton, { emoji: "\uD83D\uDDD1\uFE0F", label: t('common.reset'), title: t('graph.reset'), onClick: () => onReset(), variant: "btn", className: "!bg-red-600 hover:!bg-red-700 border-red-600 text-white px-3 py-1 rounded-lg" }))] })), _jsxs("div", { className: "relative w-full h-16", children: [_jsxs("svg", { viewBox: "0 0 100 100", className: "absolute inset-0 w-full h-full", preserveAspectRatio: "none", children: [n >= 2 && (_jsx("line", { x1: 8, y1: Y, x2: 100 - 8, y2: Y, stroke: "var(--color-border)", strokeWidth: "2", strokeLinecap: "round" })), n >= 2 && xs.slice(0, -1).map((x, i) => {
                        ...
                        return (_jsxs("div", { className: "absolute -translate-x-1/2 -translate-y-1/2 select-none text-xl group flex items-center", style: { left: `${mid}%`, top: '50%' }, children: [_jsx("span", { children: emoji }), _jsx("span", { className: "ml-1 max-w-0 opacity-0 overflow-hidden whitespace-nowrap \r\n                           group-hover:max-w-[100px] group-hover:opacity-100 \r\n                           transition-all duration-200 ease-out text-sm text-[var(--color-accent)]", children: label })] }, `icon-${i}`));
                    }), stops.map((s, i) => {
                        const isActive = s.id === activeStopId;
                        return (_jsxs("div", { className: "absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center cursor-pointer", style: { left: `${xs[i]}%`, top: '50%' }, onClick: () => handleNodeClick(i), children: [_jsx("span", { className: `w-9 h-9 rounded-full border-2 flex items-center justify-center 
                  ${isActive
                                        ? 'bg-white border-[var(--color-brand)]'
                                        : 'bg-white border-gray-300 opacity-60'}`, children: _jsx("span", { className: `inline-block rounded-full w-3.5 h-3.5 
                    ${isActive ? 'bg-[var(--color-brand)]' : 'bg-gray-300'}` }) }), _jsx("span", { className: `mt-2 text-[11px] leading-tight max-w-[120px] text-center truncate 
                  ${isActive ? 'text-[var(--color-accent)]' : 'text-gray-400'}`, children: s.city || `Stop ${i + 1}` })] }, s.id));
                    })] })] }));
}
